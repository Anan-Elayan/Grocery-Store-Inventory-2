...
public class UpdateItemPane extends MainScene {
	...
	class HandelarSearchButton implements EventHandler<ActionEvent> {
		...
		public void handle(ActionEvent arg0) {
			...
			if (txtFieldType.getText().trim().isEmpty()) {//if the text filed type is empty
				new Warning("Please set the type");
				return;
			}
			if (comboBox.getValue().equals("Brand")) {
				if (txtBrandName.getText().trim().isEmpty()) {
					new Warning("Please set the Brand Name");//if the text filed brand name is empty
					return;
				}
				...
				try {
					...
					if (item != null) {
						txtFieldQuantity.setText(Integer.toString(item.getQuantity()));//return quantity in text filed quantity
						txtFieldPrice.setText(Double.toString(item.getPrice()));//return price in text filed price
